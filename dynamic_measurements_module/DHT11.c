...
const Timer_A_UpModeConfig sampleRateTimerConfig =
{
     TIMER_A_CLOCKSOURCE_SMCLK,
     TIMER_A_CLOCKSOURCE_DIVIDER_64,
     SAMPLE_RATE_TIMER_PERIOD,
     TIMER_A_TAIE_INTERRUPT_DISABLE,
     TIMER_A_CCIE_CCR0_INTERRUPT_ENABLE,
     TIMER_A_DO_CLEAR
};
...
int main(void)
{
    ...
    MAP_Timer_A_configureUpMode(TIMER_A3_BASE, &sampleRateTimerConfig);         // sets the sensor sample rate
    ...
    /* Get Results */
    relativeHumidity = humIntBuff[0];
    temperature = tempIntBuff[0];
    ...
    MAP_Timer_A_initCapture(TIMER_A3_BASE, &captureModeConfig);

    /* Configuring Up/Down Mode */
    MAP_Timer_A_configureUpDownMode(TIMER_A3_BASE, &upDownModeConfig);

    /* Enabling interrupts and going to sleep */
    MAP_Interrupt_enableInterrupt(INT_TA2_N);
    MAP_Timer_A_enableInterrupt(TIMER_A2_BASE);

    /* Starting the TIMER_A3 in up/down mode */
    MAP_Timer_A_startCounter(TIMER_A2_BASE, TIMER_A_UPDOWN_MODE);
...
void TA2_N_IRQHandler(void)
{
    ...
    captureValues[0] =
            MAP_Timer_A_getCaptureCompareCount(TIMER_A2_BASE,
                                             TIMER_A_CAPTURECOMPARE_REGISTER_1);
    ...
    if (threshold) {
        ...
        for (kk = 0; kk < CAPTURE_VALUES_BUFFER_CAPACITY - 1; kk++) {
            captureValuesTimeDifference[kk] = abs((captureValues[kk] - captureValues[kk + 1])) / 3.0E6;     // 3.0E6 is due to SMCLK = 3MHz
        }
...
void convertToValues()
{
    ...
    for (i = 7; i >= 0; i--) {
        if (bits[7 - i] == 1) {
            humIntBuff[transmissions] += pow(2.0, i);
            LCD_busy2(4, 3, 6, 3, 7, 3, 5);
            clearLCD(4, 3, 6, 3, 7, 3, 5);
            LCD_busy2(4, 3, 6, 3, 7, 3, 5);
            writeCommand(4, 3, 6, 3, 7, 3, 5, 'n', "Humidity: ");
            LCD_busy2(4, 3, 6, 3, 7, 3, 5);
            showNumber(4, 3, 6, 3, 7, 3, 5, humIntBuff[transmissions]);
            LCD_busy2(4, 3, 6, 3, 7, 3, 5);
    ...
    for (i = 15; i >= 8; i--) {
        if (bits[23 - i] == 1) {
            humDecBuff[transmissions] += pow(2.0, i - 8);
        }
    }
    for (i = 23; i >= 16; i--) {
        if (bits[39 - i] == 1) {
            tempIntBuff[transmissions] += pow(2.0, i - 16);
            /*
            LCD_busy2(4, 3, 6, 3, 7, 3, 5);
            clearLCD(4, 3, 6, 3, 7, 3, 5);
            LCD_busy2(4, 3, 6, 3, 7, 3, 5);
    ...
    for (i = 31; i >= 24; i--) {
        if (bits[55 - i] == 1) {
            tempDecBuff[transmissions] += pow(2.0, i - 24);
        }
    }
    for (i = 39; i >= 32; i--) {
        if (bits[71 - i] == 1) {
            checkSumBuff[transmissions] += pow(2.0, i - 32);
        }
    }
}
